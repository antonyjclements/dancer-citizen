...
- The React app sets English document language and default metadata in `apps/react-site/index.html`.
- The HTML-shell Lambda replaces generic metadata with route-specific metadata for document routes before hydration.
- Global CSS defines display/body fonts, paper backgrounds, ink text, muted text, faint text, and rule color.
...
- The React app provides the public header, navigation, footer, and global CSS while preserving the top-level routes.
...
3. React Router routes to the target page.
...
- The React delivery path returns route-specific metadata for document URLs before client-side rendering begins.
...
- The README describes the React/Lambda/CDK delivery path.
- The legacy Next shell has been removed.
